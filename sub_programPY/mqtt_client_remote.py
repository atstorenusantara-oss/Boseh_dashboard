...
def start_mqtt_client(rental_callback):
    ...
    while True:
        ...
        if not client_id or not token:
            print("[MQTT Remote] Credentials not found. Retrying in 5s...")
            time.sleep(5)
            continue
        ...
        try:
            print(f"[MQTT Remote] Connecting to {host}:{port} as {mqtt_client_id} (Station: {current_client_id})...")
            ...
            while True:
                ...
                if new_client_id != current_client_id or new_token != current_token:
                    print("[MQTT Remote] Credentials changed! Reconnecting...")
                    client.loop_stop()
                    client.disconnect()
